package com.backend.lab.domain.print.entity.dto.resp;

import com.backend.lab.domain.uploadFile.entity.dto.resp.UploadFileResp;
import java.util.ArrayList;
import java.util.List;
import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Getter;
import lombok.NoArgsConstructor;

@Getter
@AllArgsConstructor
@NoArgsConstructor
@Builder
public class PrintResp {
  private String lastPageMainTitle;
  private String lastPageMainColor;

  private String lastPageSubTitle;
  private String lastPageSubColor;

  private List<UploadFileResp> firstPageImages = new ArrayList<>();
  private List<UploadFileResp> lastPageImages = new ArrayList<>();

}
